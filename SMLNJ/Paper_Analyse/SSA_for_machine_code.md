...
论文中出现过的，未解释或未详细解释的，但默认读者已完全理解的一些概念或者数学表示  
  
- y <- op(x1, x2)  
	- 通过操作符op对x1，x2操作数进行运算，将结果存储至y中  
		  
- must\_def\[y\] = r  
	- 某指令产生的结果y，必须绑定到寄存器r  
		  
- must\_use\[i\]\[k\] = r  
	- 指令i的第k个操作数，必须绑定到寄存器r  
		  
- 固定寄存器  
	- 表示某些情况下必须使用的寄存器，例如DIV指令在某些情况下会强制指定RDX:RAX寄存器对存储被除数或结果（余数和商），那么此时相对于DIV指令的固定寄存器即此寄存器对。“固定”二字偏向于指令粒度。  
		  
- 专用寄存器  
	- 与固定寄存器的概念类似，在论文中有“混用”情况，但在不同上下文中表达的含义与作用不同。专用寄存器的概念是相对于代码块上下文的，例如ebp以及esp寄存器通常被编译器定义为当前函数上下文的帧指针以及栈指针，那么ebp以及esp在当前上下文为专用寄存器；在x64的调用约定中，RCX，RDX，R8以及R9寄存器作为前四个入参，那么这四个寄存器在代码块入口处的上下文为专用寄存器。“专用”二字偏向于上下文粒度。  
		  
- 图论  
	- 尤其是支配，直接支配，严格支配的概念，在第四章的具体算法设计极其重要  
	 https://en.wikipedia.org/wiki/Dominator\_(graph\_theory) 图论基础概念  
	 https://misailo.web.engr.illinois.edu/courses/526-sp17/lec4.pdf CS526 高级编译器构造 图论课件  
		  
	- 用AI绘制了一份速查表，方便快速回忆和关联上下文：  
...
## 1. 支配者 (Dominator)  
**定义**: D支配N ⇔ 到N的所有路径都必须经过D    
**公式**: dom(n) = {n} ∪ (⋂ dom(p) for p ∈ pred(n))    
**解读**: 我的支配者 = 我自己 + 所有前驱节点支配者的公共部分  
**例子**:   
[A] → [B] → [C]  
- dom(A) = {A}  
- dom(B) = {A,B}     // 到B必须经A,B  
- dom(C) = {A,B,C}   // 到C必须经A,B,C  
  
## 2. 严格支配者 (Strict Dominator)  
**定义**: D严格支配N ⇔ D支配N且D≠N    
**公式**: strict_dom(N) = dom(N) - {N}    
**解读**: 我的严格支配者 = 我的支配者 - 我自己  
**例子**:  
[A] → [B] → [C]  
- strict_dom(A) = ∅  
- strict_dom(B) = {A}      // A支配B且A≠B  
- strict_dom(C) = {A,B}    // A,B支配C且≠C  
  
## 3. 直接支配者 (Immediate Dominator)  
**定义**: 离N最近的那个严格支配者    
**记作**: idom(N)    
**性质**: 支配树中N的父节点    
**例子**:  
...
## 4. 支配边界 (Dominance Frontier)  
**定义**: 满足以下条件的节点Y的集合：X支配Y的某个前驱节点，但X不严格支配Y  
**公式**: DF(X) = {Y | ∃p ∈ pred(Y) ∧ X dom p ∧ ¬(X strict_dom Y)}  
**解读**: X能控制进入Y的某条路径，但不完全控制Y本身。总之，支配边界就是变量定义失去唯一性、必须插入Φ函数合并值的位置集合。  
...
- DF⁺  
	- DF表示支配边界（Dominance Frontiers），符号⁺代表前者的迭代，那么DF⁺代表迭代支配边界  
...
![](5.png)  
我们通过扫描基本块并获取以下局部数据来初始化分析流程：  
  
- last\[b\]\[r\]：块b中寄存器r最后引用的值，若为空则标记为⊤  
	  
- phi\[b\]\[r\]：通过Φ函数进入块b时存储在寄存器r中的值  
	  
- sites\[r\]：所有将物理寄存器r定义或使用为固定寄存器的块  
	  
  
处理过程如Figure 5所示。通过这些辅助表，我们为所有基本块以及每个专用寄存器r∈R计算以下信息：  
  
- avin\[b\]\[r\]：块b开始时寄存器r中可用的值，如果没有则为⊥  
	  
- avout\[b\]\[r\]：块b结束时寄存器r中可用的值，如果没有则为⊥  
	  
  
数组avout和avin是以下数据流方程的最大不动点：  
![](6.png)  
...
>     ⊥ if b is the entry  
>     x if phi [i][r] = x ≠ ⊤  
>     Π{b' ∈ pred { cfg }(b)} avout[b'][r] if b ∈ DF⁺(sites[r])  
>     avout [idom(b)][r] otherwise  
> }  
>   
> avout[b][r] =   
> {  
...
> ```  
>   
> 把上面的数据流方程抽取为可复制的数学表达式，实际上该方程表示了SSA活跃变量分析的一系列基本条件。下面对两个数组的定义逐条解析：  
>   
> - avin\[b\]\[r\]  
> 	- ⊥ if b is the entry：如果块b为程序的入口（entry），则没有必要定义具体值（表现为⊥），因为一般认为程序入口处不会有活跃数据  
> 		  
> 	- x if phi \[i\]\[r\] = x ≠ ⊤：如果是通过Φ函数进入当前代码块，则判断Φ函数是否有数据定义，若有则定义当前代码块入口处的活跃数据与Φ函数的数据一致。这里其实很好理解，由于多个前驱块可能产生不同的值，算法难以保证接下来使用的确定值，因此通过Φ函数直接合并了前驱块的所有值为x以避免取值冲突，该值成为了入口处的唯一有效来源，自然无法丢弃。  
> 		  
> 	- Π{b' ∈ pred { cfg }(b)} avout\[b'\]\[r\] if b ∈ DF⁺(sites\[r\])：如果块b在sites\[r\]的迭代支配边界集合中，则块b入口处的活跃数据为块b所有前驱块出口处的活跃数据的交集  
> 		  
